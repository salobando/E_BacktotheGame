// 1. SEGURIDAD: Esto se ejecuta de primero para bloquear intrusos
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- LÓGICA DEL BOTÓN SALIR (Añadido) ---
    ...
    if (nombreUsuario && mensajeBienvenida && spanNombre) {
    ...
    listaEl.addEventListener("change", e => {
        if (e.target.classList.contains("qty-input")) {
            ...
            if (!Number.isInteger(val) || val <= 0) { 
                alert("Cantidad inválida."); e.target.value = 1; val = 1; 
            }
            const carrito = obtenerCarrito();
...
// 3. FUNCIÓN GLOBAL DE CERRAR SESIÓN
